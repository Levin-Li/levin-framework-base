...
import com.levin.oak.base.controller.base.accesswhitelist.*;
...
* 访问白名单业务控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月10日 下午3:39:30, 代码生成哈希校验码：[8d2c0ed2d6c371250b296e2947e8b56b]，请不要修改和删除此行内容。
*
*/

//生成的控制器
@RestController(PLUGIN_PREFIX + "BizAccessWhitelistController")
@RequestMapping(API_PATH + "AccessWhitelist") //accesswhitelist

@ConditionalOnProperty(prefix = PLUGIN_PREFIX, name = "BizAccessWhitelistController", havingValue = "true", matchIfMissing = true)
...
@Slf4j
public class BizAccessWhitelistController extends AccessWhitelistController{
    ...
    * @return  ApiResp<StatAccessWhitelistReq.Result>
    */
    @GetMapping("stat") //默认开放
    @Operation(summary = STAT_ACTION, description = STAT_ACTION + " " + BIZ_NAME)
    public ApiResp<StatAccessWhitelistReq.Result> stat(@Valid StatAccessWhitelistReq req, SimplePaging paging) {

        req = checkRequest(STAT_ACTION, req);

        return ApiResp.ok(checkResponse(STAT_ACTION, bizAccessWhitelistService.stat(req, paging)));
    }
