...
* 应用接入控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2023年12月7日 下午9:11:56, 代码生成哈希校验码：[f4fbdc730c1f4300381196d857709791]，请不要修改和删除此行内容。
*
*/
...
//默认需要权限访问，默认从父类继承
@ResAuthorize(domain = ID, type = SYS_TYPE_NAME + "-" + E_AppClient.BIZ_NAME)
...
public abstract class AppClientController extends BaseController{
    ...
    public ApiResp<AppClientInfo> retrieve(@NotNull @Valid AppClientIdReq req, @PathVariable(required = false) String id) {
         ...
         AppClientInfo info = appClientService.findById(req);
