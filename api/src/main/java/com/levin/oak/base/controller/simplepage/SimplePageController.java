 ...
 * 简单页面控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:46, 代码生成哈希校验码：[324b4a8506b27daf765a406e80a40bda]，请不要修改和删除此行内容。
 */
public class SimplePageController extends BaseController {
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<SimplePageInfo>> queryList(
            @Form QuerySimplePageReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatSimplePageReq.Result>> stat(
            StatSimplePageReq req, SimplePaging paging) {
    ...
    public ApiResp<SimplePageInfo> retrieve(
            @NotNull SimplePageIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateSimplePageReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody SimplePageIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
