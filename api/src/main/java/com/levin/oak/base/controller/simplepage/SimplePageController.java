 ...
 * 代码生成哈希校验码：[f030a98e830c35725dc4183f4c568d22], 请不要修改和删除此行内容。
 */
public class SimplePageController extends BaseController{
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<SimplePageInfo>> queryList(@Form QuerySimplePageReq req, SimplePaging paging) {
     ...
     public ApiResp<PagingData<StatSimplePageReq.Result>> stat(StatSimplePageReq req, SimplePaging paging) {
    ...
    */
    ...
    @CRUD.Op
    public ApiResp<SimplePageInfo> retrieve(@NotNull SimplePageIdReq req, @PathVariable(required = false) String id) {
     ...
     */
     ...
     @CRUD.Op
     public ApiResp<Boolean> update(@RequestBody UpdateSimplePageReq req, @PathVariable(required = false) String id) {
         req.updateIdWhenNotBlank(id);
         return ApiResp.ok(checkResult(simplePageService.update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @CRUD.Op
    public ApiResp<Boolean> delete(SimplePageIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(checkResult(simplePageService.delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete2(@RequestBody SimplePageIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
     ...
     */
     ...
     public ApiResp<Integer> batchUpdate(@RequestBody List<UpdateSimplePageReq> reqList) {
        return ApiResp.ok(checkResult(simplePageService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull DeleteSimplePageReq req) {
        return ApiResp.ok(checkResult(simplePageService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @Operation(summary = BATCH_DELETE_ACTION, description = BATCH_DELETE_ACTION + " " + BIZ_NAME)
    public ApiResp<Integer> batchDelete2(@RequestBody DeleteSimplePageReq req) {
