package com.levin.oak.base.controller.base.area;
...
* 区域控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月11日 下午1:45:04, 代码生成哈希校验码：[17e08eba16ee0d59b3ab5a262ffe1e00]，请不要修改和删除此行内容。
*
*/
...
public abstract class AreaController extends BaseController{
    ...
    public ApiResp<PagingData<AreaInfo>> list(@Form @Valid QueryAreaReq req, SimplePaging paging) {
        ...
        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, getAreaService().query(req, paging)));
    }
    ...
    public ApiResp<String> create(@RequestBody @Valid CreateAreaReq req) {
        ...
        return ApiResp.ok(getAreaService().create(req));
    }
    ...
    public ApiResp<AreaInfo> retrieve(@NotNull @Valid AreaIdReq req, @PathVariable(required = false) String id) {
         ...
         AreaInfo info = getAreaService().findById(req);
    ...
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateAreaReq req, @PathVariable(required = false) String id) {
        ...
        return ApiResp.ok(assertTrue(getAreaService().update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete(@Valid AreaIdReq req, @PathVariable(required = false) String id) {
        ...
        return ApiResp.ok(assertTrue(getAreaService().delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<List<String>> batchCreate(@RequestBody @Valid List<CreateAreaReq> reqList) {
        ...
        return ApiResp.ok(getAreaService().batchCreate(reqList));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateAreaReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(getAreaService().batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteAreaReq req) {
        ...
        return ApiResp.ok(assertTrue(getAreaService().batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
