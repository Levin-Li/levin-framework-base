package com.levin.oak.base.controller.base.accesslog;
...
* 访问日志控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月11日 下午1:45:04, 代码生成哈希校验码：[06884211426906072c2e41a919cdea2c]，请不要修改和删除此行内容。
*
*/
...
public abstract class AccessLogController extends BaseController{
    ...
    public ApiResp<PagingData<AccessLogInfo>> list(@Form @Valid QueryAccessLogReq req, SimplePaging paging) {
        ...
        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, getAccessLogService().query(req, paging)));
    }
    ...
    public ApiResp<Long> create(@RequestBody @Valid CreateAccessLogReq req) {
        ...
        return ApiResp.ok(getAccessLogService().create(req));
    }
    ...
    public ApiResp<AccessLogInfo> retrieve(@NotNull @Valid AccessLogIdReq req, @PathVariable(required = false) Long id) {
         ...
         AccessLogInfo info = getAccessLogService().findById(req);
    ...
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateAccessLogReq req, @PathVariable(required = false) Long id) {
        ...
        return ApiResp.ok(assertTrue(getAccessLogService().update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete(@Valid AccessLogIdReq req, @PathVariable(required = false) Long id) {
        ...
        return ApiResp.ok(assertTrue(getAccessLogService().delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<List<Long>> batchCreate(@RequestBody @Valid List<CreateAccessLogReq> reqList) {
        ...
        return ApiResp.ok(getAccessLogService().batchCreate(reqList));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateAccessLogReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(getAccessLogService().batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteAccessLogReq req) {
        ...
        return ApiResp.ok(assertTrue(getAccessLogService().batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
