 ...
 * 平台租户控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月25日 14:24:48, 请不要修改和删除此行内容。
 * 代码生成哈希校验码：[4fcbcebe373bfde28023893e5a31dd0a], 请不要修改和删除此行内容。
 */
public class TenantController extends BaseController{
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<TenantInfo>> queryList(@Form QueryTenantReq req, SimplePaging paging) {
    ...
    @CRUD.Op
    public ApiResp<TenantInfo> retrieve(@NotNull TenantIdReq req, @PathVariable(required = false) String id) {
    ...
    @CRUD.Op
    public ApiResp<Boolean> update(@RequestBody UpdateTenantReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
    ...
    public ApiResp<Boolean> delete2(@RequestBody TenantIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody List<UpdateTenantReq> reqList) {
        return ApiResp.ok(checkResult(tenantService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull DeleteTenantReq req) {
        return ApiResp.ok(checkResult(tenantService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
