package com.levin.oak.base.controller.base.notice;
...
* 通知控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月11日 下午1:45:03, 代码生成哈希校验码：[aae7fb4e2e1996128cdb495265156155]，请不要修改和删除此行内容。
*
*/
...
public abstract class NoticeController extends BaseController{
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<NoticeInfo>> list(@Form @Valid QueryNoticeReq req, SimplePaging paging) {

        req = checkRequest(QUERY_LIST_ACTION, req);

        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, getNoticeService().query(req, paging)));
    }
    ...
    public ApiResp<String> create(@RequestBody @Valid CreateNoticeReq req) {
        ...
        return ApiResp.ok(getNoticeService().create(req));
    }
    ...
    @CRUD.Op
    public ApiResp<NoticeInfo> retrieve(@NotNull @Valid NoticeIdReq req, @PathVariable(required = false) String id) {

         req.updateIdWhenNotBlank(id);

         req = checkRequest(VIEW_DETAIL_ACTION, req);

         NoticeInfo info = getNoticeService().findById(req);
    ...
    @CRUD.Op
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateNoticeReq req, @PathVariable(required = false) String id) {

        req.updateIdWhenNotBlank(id);

        req = checkRequest(UPDATE_ACTION, req);

        return ApiResp.ok(assertTrue(getNoticeService().update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @CRUD.Op
    public ApiResp<Boolean> delete(@Valid NoticeIdReq req, @PathVariable(required = false) String id) {

        req.updateIdWhenNotBlank(id);

        req = checkRequest(DELETE_ACTION, req);

        return ApiResp.ok(assertTrue(getNoticeService().delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete2(@RequestBody @Valid NoticeIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<List<String>> batchCreate(@RequestBody @Valid List<CreateNoticeReq> reqList) {
        ...
        return ApiResp.ok(getNoticeService().batchCreate(reqList));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateNoticeReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(getNoticeService().batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteNoticeReq req) {
        ...
        return ApiResp.ok(assertTrue(getNoticeService().batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
