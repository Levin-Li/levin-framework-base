...
* 通知控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2023年11月28日 下午3:31:16, 代码生成哈希校验码：[cd38f23210b1074871a3ca6a2ccbbc48]，请不要修改和删除此行内容。
*
*/
...
public abstract class NoticeController extends BaseController{
    ...
    public ApiResp<PagingData<NoticeInfo>> list(@Form @Valid QueryNoticeReq req, SimplePaging paging) {
        ...
        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, noticeService.query(req,paging)));
    }
     ...
     public ApiResp<PagingData<StatNoticeReq.Result>> stat(@Valid StatNoticeReq req, SimplePaging paging) {
         ...
         return ApiResp.ok(checkResponse(STAT_ACTION, noticeService.stat(req,paging)));
     }
    ...
    public ApiResp<NoticeInfo> retrieve(@NotNull @Valid NoticeIdReq req, @PathVariable(required = false) String id) {
         ...
         NoticeInfo info = noticeService.findById(req);
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateNoticeReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(noticeService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteNoticeReq req) {
        ...
        return ApiResp.ok(assertTrue(noticeService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
