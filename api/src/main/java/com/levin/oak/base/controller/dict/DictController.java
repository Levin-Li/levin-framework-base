 ...
 * 字典控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年8月11日 下午5:40:24, 代码生成哈希校验码：[85df6f5734d12cdc239270f54af9f9f7]，请不要修改和删除此行内容。
 */
public class DictController extends BaseController {
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<DictInfo>> queryList(
            @Form @Valid QueryDictReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatDictReq.Result>> stat(
            @Valid StatDictReq req, SimplePaging paging) {
    ...
    @CRUD.Op
    public ApiResp<DictInfo> retrieve(
            @NotNull @Valid DictIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody @Valid DictIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
