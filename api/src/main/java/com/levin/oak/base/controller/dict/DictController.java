 ...
 * 字典控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:43, 代码生成哈希校验码：[b6e884a9abf7d8b67a514379a39dd73b]，请不要修改和删除此行内容。
 */
public class DictController extends BaseController {
    ...
    public ApiResp<DictInfo> retrieve(
            @NotNull DictIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateDictReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody DictIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
