 ...
 * 应用错误日志控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:43, 代码生成哈希校验码：[ebb1752a8a2dc16daec1cf45915fd8e6]，请不要修改和删除此行内容。
 */
public class AppErrorLogController extends BaseController {
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<AppErrorLogInfo>> queryList(
            @Form QueryAppErrorLogReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatAppErrorLogReq.Result>> stat(
            StatAppErrorLogReq req, SimplePaging paging) {
    ...
    @CRUD.Op
    public ApiResp<AppErrorLogInfo> retrieve(
            @NotNull AppErrorLogIdReq req, @PathVariable(required = false) Long id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateAppErrorLogReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody AppErrorLogIdReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
