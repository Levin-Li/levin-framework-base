 ...
 * 应用错误日志控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年8月11日 下午5:40:24, 代码生成哈希校验码：[7346d2e6aafc5041a332fb1aba7a55cf]，请不要修改和删除此行内容。
 */
public class AppErrorLogController extends BaseController {
    ...
    public ApiResp<PagingData<AppErrorLogInfo>> queryList(
            @Form @Valid QueryAppErrorLogReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatAppErrorLogReq.Result>> stat(
            @Valid StatAppErrorLogReq req, SimplePaging paging) {
    ...
    public ApiResp<AppErrorLogInfo> retrieve(
            @NotNull @Valid AppErrorLogIdReq req, @PathVariable(required = false) Long id) {
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody @Valid AppErrorLogIdReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
