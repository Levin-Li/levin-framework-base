package com.levin.oak.base.controller.base.tenantapp;
...
* 租户应用控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月11日 下午1:45:03, 代码生成哈希校验码：[84d5ed175d7452cc0cc9f43c5e558ed7]，请不要修改和删除此行内容。
*
*/
...
//@ConditionalOnProperty(prefix = PLUGIN_PREFIX, name = "TenantAppController", havingValue = "true",  matchIfMissing = true)
...
public abstract class TenantAppController extends BaseController{
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<TenantAppInfo>> list(@Form @Valid QueryTenantAppReq req, SimplePaging paging) {

        req = checkRequest(QUERY_LIST_ACTION, req);

        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, getTenantAppService().query(req, paging)));
    }
    ...
    public ApiResp<String> create(@RequestBody @Valid CreateTenantAppReq req) {
        ...
        return ApiResp.ok(getTenantAppService().create(req));
    }
    ...
    @CRUD.Op
    public ApiResp<TenantAppInfo> retrieve(@NotNull @Valid TenantAppIdReq req, @PathVariable(required = false) String id) {

         req.updateIdWhenNotBlank(id);

         req = checkRequest(VIEW_DETAIL_ACTION, req);

         TenantAppInfo info = getTenantAppService().findById(req);
    ...
    @CRUD.Op
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateTenantAppReq req, @PathVariable(required = false) String id) {

        req.updateIdWhenNotBlank(id);

        req = checkRequest(UPDATE_ACTION, req);

        return ApiResp.ok(assertTrue(getTenantAppService().update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @CRUD.Op
    public ApiResp<Boolean> delete(@Valid TenantAppIdReq req, @PathVariable(required = false) String id) {

        req.updateIdWhenNotBlank(id);

        req = checkRequest(DELETE_ACTION, req);

        return ApiResp.ok(assertTrue(getTenantAppService().delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete2(@RequestBody @Valid TenantAppIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<List<String>> batchCreate(@RequestBody @Valid List<CreateTenantAppReq> reqList) {
        ...
        return ApiResp.ok(getTenantAppService().batchCreate(reqList));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateTenantAppReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(getTenantAppService().batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteTenantAppReq req) {
        ...
        return ApiResp.ok(assertTrue(getTenantAppService().batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
