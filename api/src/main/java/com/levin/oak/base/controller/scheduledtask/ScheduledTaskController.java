 ...
 * 调度任务控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:45, 代码生成哈希校验码：[fa681d96472f59c14a4b54fdfd2efb1f]，请不要修改和删除此行内容。
 */
public class ScheduledTaskController extends BaseController {
    ...
    public ApiResp<PagingData<ScheduledTaskInfo>> queryList(
            @Form QueryScheduledTaskReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatScheduledTaskReq.Result>> stat(
            StatScheduledTaskReq req, SimplePaging paging) {
    ...
    public ApiResp<ScheduledTaskInfo> retrieve(
            @NotNull ScheduledTaskIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateScheduledTaskReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(
    ...
    public ApiResp<Boolean> delete(
            ScheduledTaskIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody ScheduledTaskIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
