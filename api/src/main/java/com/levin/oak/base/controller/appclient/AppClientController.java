 ...
 * 应用接入控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:42, 代码生成哈希校验码：[4e091fc925d13dc2a0bc715e9d6d2b95]，请不要修改和删除此行内容。
 */
public class AppClientController extends BaseController {
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<AppClientInfo>> queryList(
            @Form QueryAppClientReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatAppClientReq.Result>> stat(
            StatAppClientReq req, SimplePaging paging) {
    ...
    @CRUD.Op
    public ApiResp<AppClientInfo> retrieve(
            @NotNull AppClientIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateAppClientReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody AppClientIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
