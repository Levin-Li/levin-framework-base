 ...
 * 应用接入控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年8月11日 下午5:40:23, 代码生成哈希校验码：[97af30cd941c3bd65098cf50310a1885]，请不要修改和删除此行内容。
 */
public class AppClientController extends BaseController {
    ...
    public ApiResp<PagingData<AppClientInfo>> queryList(
            @Form @Valid QueryAppClientReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatAppClientReq.Result>> stat(
            @Valid StatAppClientReq req, SimplePaging paging) {
    ...
    public ApiResp<AppClientInfo> retrieve(
            @NotNull @Valid AppClientIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody @Valid AppClientIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
