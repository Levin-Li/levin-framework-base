package com.levin.oak.base.controller.base.user;
...
* 用户控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月11日 下午1:45:03, 代码生成哈希校验码：[ebf17391468a94af11859d8e86a91917]，请不要修改和删除此行内容。
*
*/
...
public abstract class UserController extends BaseController{
    ...
    protected static final String BIZ_NAME = E_User.BIZ_NAME;
    ...
    public ApiResp<PagingData<UserInfo>> list(@Form @Valid QueryUserReq req, SimplePaging paging) {
        ...
        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, getUserService().query(req, paging)));
    }
    ...
    public ApiResp<String> create(@RequestBody @Valid CreateUserReq req) {
        ...
        return ApiResp.ok(getUserService().create(req));
    }
    ...
    public ApiResp<UserInfo> retrieve(@NotNull @Valid UserIdReq req, @PathVariable(required = false) String id) {
         ...
         UserInfo info = getUserService().findById(req);
    ...
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateUserReq req, @PathVariable(required = false) String id) {
        ...
        return ApiResp.ok(assertTrue(getUserService().update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete(@Valid UserIdReq req, @PathVariable(required = false) String id) {
        ...
        return ApiResp.ok(assertTrue(getUserService().delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<List<String>> batchCreate(@RequestBody @Valid List<CreateUserReq> reqList) {

        reqList = checkRequest(BATCH_CREATE_ACTION, reqList);

        return ApiResp.ok(getUserService().batchCreate(reqList));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateUserReq> reqList) {

        reqList = checkRequest(BATCH_UPDATE_ACTION, reqList);

        return ApiResp.ok(assertTrue(getUserService().batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteUserReq req) {

        req = checkRequest(BATCH_DELETE_ACTION, req);

        return ApiResp.ok(assertTrue(getUserService().batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }

    /**
     * 批量删除2
     * @param req @RequestBody DeleteUserReq
     */
    @DeleteMapping(value = {"batchDelete"}, consumes = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = BATCH_DELETE_ACTION, description = BATCH_DELETE_ACTION + " " + BIZ_NAME)
    public ApiResp<Integer> batchDelete2(@RequestBody @Valid DeleteUserReq req) {
