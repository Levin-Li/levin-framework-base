...
* 用户控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2023年11月26日 上午8:27:05, 代码生成哈希校验码：[36274e042da3138bff05a3183be16d76]，请不要修改和删除此行内容。
*
*/
...
public abstract class UserController extends BaseController{
    ...
    public ApiResp<PagingData<UserInfo>> list(@Form @Valid QueryUserReq req, SimplePaging paging) {
     ...
     public ApiResp<PagingData<StatUserReq.Result>> stat(@Valid StatUserReq req, SimplePaging paging) {
         return ApiResp.ok(userService.stat(req,paging));
     }
    ...
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateUserReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(assertTrue(userService.update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete(@Valid UserIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(assertTrue(userService.delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateUserReq> reqList) {
        return ApiResp.ok(assertTrue(userService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteUserReq req) {
        return ApiResp.ok(assertTrue(userService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
