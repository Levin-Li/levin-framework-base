...
@Tag(name = "访问日志", description = "访问日志管理")
@Slf4j @Valid
public class AccessLogController {

    private static final String ENTITY_NAME ="访问日志";
    ...
    */
     ...
     */
     ...
     public ApiResp<Void> update(@PathVariable Long id , @RequestBody UpdateAccessLogReq req) {
         //路径参数优先使用
         if (isNotEmpty(id)) { req.setId(id); }
         return accessLogService.update(req) > 0 ? ApiResp.ok() : ApiResp.error("更新" + ENTITY_NAME + "失败");
    }
    ...
    public ApiResp<Void> delete(@PathVariable Long id , DeleteAccessLogReq req) {
        //路径参数优先使用
        if (isNotEmpty(id)) { req.setId(id); }
        return accessLogService.delete(req) > 0 ? ApiResp.ok() : ApiResp.error("删除" + ENTITY_NAME + "失败");
    }
