 ...
 * 代码生成哈希校验码：[537e2c4def7134cce9e1f080415d4ba1], 请不要修改和删除此行内容。
 */
public class AccessLogController extends BaseController{
    ...
    //@Autowired
    @DubboReference
    BizAccessLogService bizAccessLogService;
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<AccessLogInfo>> queryList(@Form QueryAccessLogReq req, SimplePaging paging) {
     ...
     public ApiResp<PagingData<StatAccessLogReq.Result>> stat(StatAccessLogReq req, SimplePaging paging) {
    ...
    */
    ...
    @CRUD.Op
    public ApiResp<AccessLogInfo> retrieve(@NotNull AccessLogIdReq req, @PathVariable(required = false) Long id) {
     ...
     */
     ...
     @CRUD.Op
     public ApiResp<Boolean> update(@RequestBody UpdateAccessLogReq req, @PathVariable(required = false) Long id) {
         req.updateIdWhenNotBlank(id);
         return ApiResp.ok(checkResult(accessLogService.update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @CRUD.Op
    public ApiResp<Boolean> delete(AccessLogIdReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(checkResult(accessLogService.delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete2(@RequestBody AccessLogIdReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
     ...
     */
     ...
     public ApiResp<Integer> batchUpdate(@RequestBody List<UpdateAccessLogReq> reqList) {
        return ApiResp.ok(checkResult(accessLogService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull DeleteAccessLogReq req) {
        return ApiResp.ok(checkResult(accessLogService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @Operation(summary = BATCH_DELETE_ACTION, description = BATCH_DELETE_ACTION + " " + BIZ_NAME)
    public ApiResp<Integer> batchDelete2(@RequestBody DeleteAccessLogReq req) {
