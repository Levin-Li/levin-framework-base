 ...
 * 访问日志控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:44, 代码生成哈希校验码：[64d5be0389fd59d35d4a0c55f84e50f8]，请不要修改和删除此行内容。
 */
public class AccessLogController extends BaseController {
    ...
    public ApiResp<PagingData<AccessLogInfo>> queryList(
            @Form QueryAccessLogReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatAccessLogReq.Result>> stat(
            StatAccessLogReq req, SimplePaging paging) {
    ...
    public ApiResp<AccessLogInfo> retrieve(
            @NotNull AccessLogIdReq req, @PathVariable(required = false) Long id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateAccessLogReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody AccessLogIdReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
