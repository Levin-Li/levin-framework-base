...
public class AccessLogController extends BaseController{
    ...
    */
    ...
    public ApiResp<Void> delete(@PathVariable @NotNull Long id) {
        return accessLogService.delete(new DeleteAccessLogReq().setId(id)) > 0
    ...
    public ApiResp<Void> batchDelete(@NotNull DeleteAccessLogReq req) {
        //new DeleteAccessLogReq().setIdList(idList)
        return accessLogService.delete(req) > 0 ? ApiResp.ok() : ApiResp.error(DELETE_ACTION + BIZ_NAME + "失败");
    }  
