package com.levin.oak.base.controller.base.org;
...
* 机构控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月11日 下午1:45:04, 代码生成哈希校验码：[4a922dd6f4cd3891c4966aa3dfd9c63b]，请不要修改和删除此行内容。
*
*/
...
public abstract class OrgController extends BaseController{
    ...
    public ApiResp<PagingData<OrgInfo>> list(@Form @Valid QueryOrgReq req, SimplePaging paging) {
        ...
        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, getOrgService().query(req, paging)));
    }
    ...
    public ApiResp<String> create(@RequestBody @Valid CreateOrgReq req) {
        ...
        return ApiResp.ok(getOrgService().create(req));
    }
    ...
    public ApiResp<OrgInfo> retrieve(@NotNull @Valid OrgIdReq req, @PathVariable(required = false) String id) {
         ...
         OrgInfo info = getOrgService().findById(req);
    ...
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateOrgReq req, @PathVariable(required = false) String id) {
        ...
        return ApiResp.ok(assertTrue(getOrgService().update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete(@Valid OrgIdReq req, @PathVariable(required = false) String id) {
        ...
        return ApiResp.ok(assertTrue(getOrgService().delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete2(@RequestBody @Valid OrgIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<List<String>> batchCreate(@RequestBody @Valid List<CreateOrgReq> reqList) {

        reqList = checkRequest(BATCH_CREATE_ACTION, reqList);

        return ApiResp.ok(getOrgService().batchCreate(reqList));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateOrgReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(getOrgService().batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteOrgReq req) {
        ...
        return ApiResp.ok(assertTrue(getOrgService().batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @Operation(summary = BATCH_DELETE_ACTION, description = BATCH_DELETE_ACTION + " " + BIZ_NAME)
    public ApiResp<Integer> batchDelete2(@RequestBody @Valid DeleteOrgReq req) {
