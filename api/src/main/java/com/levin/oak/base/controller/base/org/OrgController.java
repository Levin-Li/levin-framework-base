...
* 机构控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2023年11月26日 上午9:43:51, 代码生成哈希校验码：[1ee1bd9f2bd0f23bd20d2ac8e34a3c78]，请不要修改和删除此行内容。
*
*/
...
public abstract class OrgController extends BaseController{
    ...
    public ApiResp<PagingData<OrgInfo>> list(@Form @Valid QueryOrgReq req, SimplePaging paging) {
        ...
        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, orgService.query(req,paging)));
    }
     ...
     public ApiResp<PagingData<StatOrgReq.Result>> stat(@Valid StatOrgReq req, SimplePaging paging) {
         ...
         return ApiResp.ok(checkResponse(STAT_ACTION, orgService.stat(req,paging)));
     }
    ...
    public ApiResp<OrgInfo> retrieve(@NotNull @Valid OrgIdReq req, @PathVariable(required = false) String id) {
         ...
         OrgInfo info = orgService.findById(req);
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateOrgReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(orgService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteOrgReq req) {
        ...
        return ApiResp.ok(assertTrue(orgService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
