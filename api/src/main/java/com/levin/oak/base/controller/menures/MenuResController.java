...
public class MenuResController {
    ...
    */
     ...
     */
     ...
     public ApiResp<Void> update(@PathVariable Long id , @RequestBody UpdateMenuResReq req) {
         //路径参数优先使用
         if (isNotEmpty(id)) { req.setId(id); }
         return menuResService.update(req) > 0 ? ApiResp.ok() : ApiResp.error("更新" + ENTITY_NAME + "失败");
    }
    ...
    public ApiResp<Void> delete(@PathVariable Long id , DeleteMenuResReq req) {
        //路径参数优先使用
        if (isNotEmpty(id)) { req.setId(id); }
        return menuResService.delete(req) > 0 ? ApiResp.ok() : ApiResp.error("删除" + ENTITY_NAME + "失败");
    }
