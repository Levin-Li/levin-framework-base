...
* 平台租户控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2023年11月26日 上午9:43:50, 代码生成哈希校验码：[a524835c226613d2e75cdbf148bd14f0]，请不要修改和删除此行内容。
*
*/
...
public abstract class TenantController extends BaseController{
    ...
    public ApiResp<TenantInfo> retrieve(@NotNull @Valid TenantIdReq req, @PathVariable(required = false) String id) {
         ...
         TenantInfo info = tenantService.findById(req);
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateTenantReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(tenantService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteTenantReq req) {
        ...
        return ApiResp.ok(assertTrue(tenantService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
