package com.levin.oak.base.controller.base.tenant;
...
* 平台租户控制器
*
* @author Auto gen by simple-dao-codegen, @time: 2024年3月11日 下午1:45:03, 代码生成哈希校验码：[afc26498929294a81215af39220567c1]，请不要修改和删除此行内容。
*
*/
...
public abstract class TenantController extends BaseController{
    ...
    public ApiResp<PagingData<TenantInfo>> list(@Form @Valid QueryTenantReq req, SimplePaging paging) {
        ...
        return ApiResp.ok(checkResponse(QUERY_LIST_ACTION, getTenantService().query(req, paging)));
    }
    ...
    public ApiResp<String> create(@RequestBody @Valid CreateTenantReq req) {
        ...
        return ApiResp.ok(getTenantService().create(req));
    }
    ...
    @CRUD.Op
    public ApiResp<TenantInfo> retrieve(@NotNull @Valid TenantIdReq req, @PathVariable(required = false) String id) {

         req.updateIdWhenNotBlank(id);

         req = checkRequest(VIEW_DETAIL_ACTION, req);

         TenantInfo info = getTenantService().findById(req);
    ...
    @CRUD.Op
    public ApiResp<Boolean> update(@RequestBody @Valid UpdateTenantReq req, @PathVariable(required = false) String id) {

        req.updateIdWhenNotBlank(id);

        req = checkRequest(UPDATE_ACTION, req);

        return ApiResp.ok(assertTrue(getTenantService().update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @CRUD.Op
    public ApiResp<Boolean> delete(@Valid TenantIdReq req, @PathVariable(required = false) String id) {

        req.updateIdWhenNotBlank(id);

        req = checkRequest(DELETE_ACTION, req);

        return ApiResp.ok(assertTrue(getTenantService().delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete2(@RequestBody @Valid TenantIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<List<String>> batchCreate(@RequestBody @Valid List<CreateTenantReq> reqList) {
        ...
        return ApiResp.ok(getTenantService().batchCreate(reqList));
    }
    ...
    public ApiResp<Integer> batchUpdate(@RequestBody @Valid List<UpdateTenantReq> reqList) {
        ...
        return ApiResp.ok(assertTrue(getTenantService().batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull @Valid DeleteTenantReq req) {
        ...
        return ApiResp.ok(assertTrue(getTenantService().batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
