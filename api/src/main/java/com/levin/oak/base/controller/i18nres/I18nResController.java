 ...
 * 国际化资源控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年8月11日 下午5:40:28, 代码生成哈希校验码：[c6284b9fbe820a3b04bc230cafe7a8b0]，请不要修改和删除此行内容。
 */
public class I18nResController extends BaseController {
    ...
    public ApiResp<PagingData<I18nResInfo>> queryList(
            @Form @Valid QueryI18nResReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatI18nResReq.Result>> stat(
            @Valid StatI18nResReq req, SimplePaging paging) {
    ...
    public ApiResp<I18nResInfo> retrieve(
            @NotNull @Valid I18nResIdReq req, @PathVariable(required = false) Long id) {
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody @Valid I18nResIdReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
