 ...
 * 国际化资源控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:45, 代码生成哈希校验码：[3f414ce82f9304cdc6d63cd91f6a5129]，请不要修改和删除此行内容。
 */
public class I18nResController extends BaseController {
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<I18nResInfo>> queryList(
            @Form QueryI18nResReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatI18nResReq.Result>> stat(
            StatI18nResReq req, SimplePaging paging) {
    ...
    @CRUD.Op
    public ApiResp<I18nResInfo> retrieve(
            @NotNull I18nResIdReq req, @PathVariable(required = false) Long id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateI18nResReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody I18nResIdReq req, @PathVariable(required = false) Long id) {
        req.updateIdWhenNotBlank(id);
