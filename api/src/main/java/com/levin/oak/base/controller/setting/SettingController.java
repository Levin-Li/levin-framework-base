...
@Tag(name = "系统设置", description = "系统设置管理")
@Slf4j @Valid
public class SettingController {

    private static final String ENTITY_NAME ="系统设置";
    ...
    */
     ...
     */
     ...
     public ApiResp<Void> update(@PathVariable Long id , @RequestBody UpdateSettingReq req) {
         //路径参数优先使用
         if (isNotEmpty(id)) { req.setId(id); }
         return settingService.update(req) > 0 ? ApiResp.ok() : ApiResp.error("更新" + ENTITY_NAME + "失败");
    }
    ...
    public ApiResp<Void> delete(@PathVariable Long id , DeleteSettingReq req) {
        //路径参数优先使用
        if (isNotEmpty(id)) { req.setId(id); }
        return settingService.delete(req) > 0 ? ApiResp.ok() : ApiResp.error("删除" + ENTITY_NAME + "失败");
    }
