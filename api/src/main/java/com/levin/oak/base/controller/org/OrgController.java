 ...
 * 机构控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年8月11日 下午5:40:27, 代码生成哈希校验码：[050fda9723258ae8db9ca80a839683f5]，请不要修改和删除此行内容。
 */
public class OrgController extends BaseController {
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<OrgInfo>> queryList(
            @Form @Valid QueryOrgReq req, SimplePaging paging) {
    ...
    @CRUD.Op
    public ApiResp<OrgInfo> retrieve(
            @NotNull @Valid OrgIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody @Valid OrgIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
