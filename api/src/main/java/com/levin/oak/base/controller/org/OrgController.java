 ...
 * 机构控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年7月27日 下午6:25:45, 代码生成哈希校验码：[995d91cbc7984391feb85d950601b711]，请不要修改和删除此行内容。
 */
public class OrgController extends BaseController {
    ...
    public ApiResp<OrgInfo> retrieve(
            @NotNull OrgIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody UpdateOrgReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody OrgIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
