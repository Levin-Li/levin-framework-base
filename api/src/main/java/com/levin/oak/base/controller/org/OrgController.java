...
public class OrgController {
    ...
    */
     ...
     */
     ...
     public ApiResp<Void> update(@PathVariable Long id , @RequestBody UpdateOrgReq req) {
         //路径参数优先使用
         if (isNotEmpty(id)) { req.setId(id); }
         return orgService.update(req) > 0 ? ApiResp.ok() : ApiResp.error("更新" + ENTITY_NAME + "失败");
    }
    ...
    public ApiResp<Void> delete(@PathVariable Long id , DeleteOrgReq req) {
        //路径参数优先使用
        if (isNotEmpty(id)) { req.setId(id); }
        return orgService.delete(req) > 0 ? ApiResp.ok() : ApiResp.error("删除" + ENTITY_NAME + "失败");
    }
