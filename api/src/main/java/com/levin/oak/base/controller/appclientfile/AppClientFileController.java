 ...
 * 代码生成哈希校验码：[9b664fcc3706803f6302159c82e998f8], 请不要修改和删除此行内容。
 */
public class AppClientFileController extends BaseController{
    ...
    //@Autowired
    @DubboReference
    BizAppClientFileService bizAppClientFileService;
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<AppClientFileInfo>> queryList(@Form QueryAppClientFileReq req, SimplePaging paging) {
    ...
    */
    ...
    @CRUD.Op
    public ApiResp<AppClientFileInfo> retrieve(@NotNull AppClientFileIdReq req, @PathVariable(required = false) String id) {
     ...
     */
     ...
     @CRUD.Op
     public ApiResp<Boolean> update(@RequestBody UpdateAppClientFileReq req, @PathVariable(required = false) String id) {
         req.updateIdWhenNotBlank(id);
         return ApiResp.ok(checkResult(appClientFileService.update(req), UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    @CRUD.Op
    public ApiResp<Boolean> delete(AppClientFileIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
        return ApiResp.ok(checkResult(appClientFileService.delete(req), DELETE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Boolean> delete2(@RequestBody AppClientFileIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
     ...
     */
     ...
     public ApiResp<Integer> batchUpdate(@RequestBody List<UpdateAppClientFileReq> reqList) {
        return ApiResp.ok(checkResult(appClientFileService.batchUpdate(reqList), BATCH_UPDATE_ACTION + BIZ_NAME + "失败"));
    }
    ...
    public ApiResp<Integer> batchDelete(@NotNull DeleteAppClientFileReq req) {
        return ApiResp.ok(checkResult(appClientFileService.batchDelete(req), BATCH_DELETE_ACTION + BIZ_NAME + "失败"));
    }
