 ...
 * 客户端文件控制器
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年11月18日 上午12:26:20, 代码生成哈希校验码：[f36d5c6a4b551f65c666bab84489ba29]，请不要修改和删除此行内容。
 */
public class AppClientFileController extends BaseController {
    ...
    @CRUD.ListTable
    public ApiResp<PagingData<AppClientFileInfo>> list(
            @Form @Valid QueryAppClientFileReq req, SimplePaging paging) {
    ...
    public ApiResp<PagingData<StatAppClientFileReq.Result>> stat(
            @Valid StatAppClientFileReq req, SimplePaging paging) {
    ...
    public ApiResp<AppClientFileInfo> retrieve(
            @NotNull @Valid AppClientFileIdReq req, @PathVariable(required = false) String id) {
    ...
    public ApiResp<Boolean> update(
            @RequestBody @Valid UpdateAppClientFileReq req,
    ...
    public ApiResp<Boolean> delete2(
            @RequestBody @Valid AppClientFileIdReq req, @PathVariable(required = false) String id) {
        req.updateIdWhenNotBlank(id);
    ...
    public ApiResp<List<String>> batchCreate(
            @RequestBody @Valid List<CreateAppClientFileReq> reqList) {
