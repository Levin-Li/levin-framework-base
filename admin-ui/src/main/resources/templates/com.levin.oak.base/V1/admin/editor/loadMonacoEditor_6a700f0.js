amis.define('loadMonacoEditor.ts', function(require, exports, module, define) {
  ...
  function filterUrl(url, filter) {
      if (filter === void 0) { filter = (function (info) { return info; }); }
      return filter(url);
  }
  function onLoad(req, callback) {
      var basePath = 'node_modules/monaco-editor/min/vs/editor/editor.main.js'.replace(/\/vs\/.*$/, '');
      console.log("basePath-0:" + basePath);
      var config = {
          'vs/nls': {
              availableLanguages: {
                  '*': 'zh-cn'
          ...
          'paths': {
              ...
              // 'vs/editor/contrib/suggest/media/String_16x.svg':echo('monaco-editor/min/vs/editor/contrib/suggest/media/String_16x.svg'),
              // 'vs/editor/contrib/suggest/media/String_inverse_16x.svg':echo('monaco-editor/min/vs/editor/contrib/suggest/media/String_inverse_16x.svg'),
              // 'vs/editor/standalone/browser/quickOpen/symbol-sprite.svg':echo('monaco-editor/min/vs/editor/standalone/browser/quickOpen/symbol-sprite.svg'),
              'vs/language/typescript/tsMode': 'node_modules/monaco-editor/min/vs/language/typescript/tsMode.js',
              // 'vs/language/typescript/lib/typescriptServices':echo('monaco-editor/min/vs/language/typescript/lib/typescriptServices.js'),
      ...
      Object.keys(config.paths).forEach(function (key) {
          ...
          //   .replace('monaco-editor/min/vs/', basePath + "/")
      });
      req.config(config);
      if (/^(https?:)?\/\//.test(config.paths.vs)) {
          console.log("basePath-1：" + config.paths.vs);
          console.log(config.paths);
          window.MonacoEnvironment = {
              getWorkerUrl: function () {
                  return "data:text/javascript;charset=utf-8,".concat(encodeURIComponent("\n              self.MonacoEnvironment = {\n                  // baseUrl: '".concat(config.paths.vs, "',\n                  baseUrl: '',\n                  paths: ").concat(JSON.stringify(config.paths), "\n              };\n              importScripts('").concat('node_modules/monaco-editor/min/vs/base/worker/workerMain.js', "');")));
              }
          };
      }
      else {
          console.log("basePath-2：" + config.paths.vs);
          console.log(config.paths);
          delete window.MonacoEnvironment;
      }
      req([config.paths.vs + '/vs/editor/editor.main'], function (ret) {
          callback(ret);
      });
  }
  //http://127.0.0.1:18081/public/node_modules/monaco-editor/min/node_modules/monaco-editor/min/vs/language/typescript/tsWorker_f8582cd.js
