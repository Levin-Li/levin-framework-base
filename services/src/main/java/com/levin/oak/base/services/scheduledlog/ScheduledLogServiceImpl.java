...
@ConditionalOnProperty(value = PLUGIN_PREFIX + "ScheduledLogService", havingValue = "false", matchIfMissing = true)
@Slf4j
//@Validated
...
public class ScheduledLogServiceImpl implements ScheduledLogService {
    ...
    @Operation(tags = {BIZ_NAME}, summary = VIEW_DETAIL_ACTION)
    @Override
    @Cacheable(condition = "#req.getCacheId() != null", unless = "#result == null ", key = E_ScheduledLog.CACHE_KEY_PREFIX + "#req.getCacheId()")
    public ScheduledLogInfo findById(QueryScheduledLogByIdReq req) {
