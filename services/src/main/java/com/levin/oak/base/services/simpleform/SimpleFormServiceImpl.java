 ...
 *  简单表单-服务实现
 *
 *@author auto gen by simple-dao-codegen 2022-3-25 13:28:16
 *
 */
...
public class SimpleFormServiceImpl extends BaseService implements SimpleFormService {
    ...
    @Transactional(rollbackFor = {PersistenceException.class, DataAccessException.class})
    @Override
    public List<Integer> batchDelete(DeleteSimpleFormReq req){
        //@Todo 优化批量提交
        return Stream.of(req.getIdList())
            .map(id -> simpleDao.copy(req, new SimpleFormIdReq().setId(id)))
            .map(idReq -> getSelfProxy().delete((SimpleFormIdReq)idReq))
