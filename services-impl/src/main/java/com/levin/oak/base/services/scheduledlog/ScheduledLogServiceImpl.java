 ...
 * 调度日志-服务实现
 *
 * @author Auto gen by simple-dao-codegen, @time: 2024年3月29日 上午12:46:07, 代码生成哈希校验码：[4321a33f4e3c66da04bdd1ca10f0fd07]，请不要修改和删除此行内容。
 *
 */

@Service(ScheduledLogService.SERVICE_BEAN_NAME)

@ConditionalOnProperty(name = ScheduledLogService.SERVICE_BEAN_NAME, havingValue = "true", matchIfMissing = true)
@Slf4j
...
@CacheConfig(cacheNames = ScheduledLogService.CACHE_NAME, cacheResolver = PLUGIN_PREFIX + "ModuleSpringCacheResolver")
...
public class ScheduledLogServiceImpl extends BaseService<ScheduledLogServiceImpl> implements ScheduledLogService {
    ...
    */
    ...
    @Transactional
    public boolean update(UpdateScheduledLogReq req, Object... queryObjs) {
    ...
    @Override
    public PagingData<ScheduledLogInfo> query(QueryScheduledLogReq req, Paging paging, Object... queryObjs) {
        return simpleDao.findPagingDataByQueryObj(req, paging, queryObjs);
    }

    @Operation(summary = QUERY_ACTION + "-指定列", description = "通常用于字段过多的情况，提升性能")
    public PagingData<ScheduledLogInfo> selectQuery(QueryScheduledLogReq req, Paging paging, String... columnNames){
        return simpleDao.forSelect(ScheduledLogInfo.class, req, paging).select(columnNames).findPaging(null, paging);
    }
    ...
    */
    ...
    /**
    * 清除[ScheduledLogService.CACHE_NAME]缓存中的所有缓存
    *
    */
    @Override
