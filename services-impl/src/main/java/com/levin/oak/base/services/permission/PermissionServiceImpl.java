 ...
 * 权限清单-服务实现
 *
 * @author Auto gen by simple-dao-codegen, @time: 2024年3月29日 上午12:46:07, 代码生成哈希校验码：[a91cccd7a01511db534beed88b0b6009]，请不要修改和删除此行内容。
 *
 */

@Service(PermissionService.SERVICE_BEAN_NAME)

@ConditionalOnProperty(name = PermissionService.SERVICE_BEAN_NAME, havingValue = "true", matchIfMissing = true)
@Slf4j
...
public class PermissionServiceImpl extends BaseService<PermissionServiceImpl> implements PermissionService {
    ...
    */
    ...
    public String create(CreatePermissionReq req){
        ...
        Permission entity = simpleDao.create(req, true);
        return entity.getId();
    }
    ...
    @Transactional
    @Override
    public List<String> batchCreate(List<CreatePermissionReq> reqList){
    ...
    @Transactional
    public boolean update(UpdatePermissionReq req, Object... queryObjs) {
    ...
    public int batchUpdate(List<UpdatePermissionReq> reqList){
        //@Todo 优化批量提交
    ...
    public int batchDelete(DeletePermissionReq req){
        //@Todo 优化批量提交
        return Stream.of(req.getIdList())
            .map(id -> simpleDao.copy(req, new PermissionIdReq().setId(id)))
    ...
    @Override
    public PagingData<PermissionInfo> query(QueryPermissionReq req, Paging paging, Object... queryObjs) {
        return simpleDao.findPagingDataByQueryObj(req, paging, queryObjs);
    }

    @Operation(summary = QUERY_ACTION + "-指定列", description = "通常用于字段过多的情况，提升性能")
    public PagingData<PermissionInfo> selectQuery(QueryPermissionReq req, Paging paging, String... columnNames){
        return simpleDao.forSelect(PermissionInfo.class, req, paging).select(columnNames).findPaging(null, paging);
    }
    ...
    public PermissionInfo findById(String id) {
        return findById(new PermissionIdReq().setId(id));
    }
    ...
    @Operation(summary = QUERY_ACTION)
    @Override
    public PermissionInfo findUnique(QueryPermissionReq req){
    ...
    */
    ...
    /**
    * 清除[PermissionService.CACHE_NAME]缓存中的所有缓存
    *
    */
    @Override
