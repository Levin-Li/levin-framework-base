...
import static com.levin.oak.base.services.jobpost.JobPostService.*;
 ...
 *  工作岗位-业务服务实现类
 *
 * @author Auto gen by simple-dao-codegen, @time: 2024年3月29日 上午1:20:15, 代码生成哈希校验码：[9b54f7df0d95ca2d2c2c3af5874b5633]，请不要修改和删除此行内容。
 *
 */
...
@Service(PLUGIN_PREFIX + "BizJobPostService")

@ConditionalOnProperty(prefix = PLUGIN_PREFIX, name = "BizJobPostService", havingValue = "true", matchIfMissing = true)
@Slf4j
...
@CacheConfig(cacheNames = JobPostService.CACHE_NAME, cacheResolver = PLUGIN_PREFIX + "ModuleSpringCacheResolver")

public class BizJobPostServiceImpl extends BaseService<BizJobPostServiceImpl> implements BizJobPostService {
    ...
    public boolean update(UpdateJobPostReq req) {
        return jobPostService.update(req);
    }
