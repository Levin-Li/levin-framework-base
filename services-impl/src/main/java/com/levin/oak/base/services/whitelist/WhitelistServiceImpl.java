package com.levin.oak.base.services.whitelist;
...
@Service(WhitelistService.SERVICE_BEAN_NAME)

@ConditionalOnProperty(name = WhitelistService.SERVICE_BEAN_NAME, havingValue = "true", matchIfMissing = true)
@Slf4j

//@Valid只能用在controller， @Validated可以用在其他被spring管理的类上。
//@Validated
@Tag(name = E_Whitelist.BIZ_NAME, description = E_Whitelist.BIZ_NAME + MAINTAIN_ACTION)
@CacheConfig(cacheNames = {ID + CACHE_DELIM + E_Whitelist.SIMPLE_CLASS_NAME}, cacheResolver = PLUGIN_PREFIX + "ModuleSpringCacheResolver")
public class WhitelistServiceImpl extends BaseService<WhitelistServiceImpl> implements WhitelistService {
    ...
    */
    ...
    @Transactional
    public boolean update(UpdateWhitelistReq req, Object... queryObjs) {
    ...
    public int batchUpdate(SimpleUpdateWhitelistReq setReq, QueryWhitelistReq whereReq){
    ...
    public int batchUpdate(List<UpdateWhitelistReq> reqList){
        //@Todo 优化批量提交
    ...
    public int batchDelete(DeleteWhitelistReq req){
        //@Todo 优化批量提交
        return Stream.of(req.getIdList())
            .map(id -> simpleDao.copy(req, new WhitelistIdReq().setId(id)))
    ...
    @Override
    public PagingData<WhitelistInfo> query(QueryWhitelistReq req, Paging paging, Object... queryObjs) {
        return simpleDao.findPagingDataByQueryObj(req, paging, queryObjs);
    }

    @Operation(summary = QUERY_ACTION + "-指定列", description = "通常用于字段过多的情况，提升性能")
    public PagingData<WhitelistInfo> selectQuery(QueryWhitelistReq req, Paging paging, String... columnNames){
        return simpleDao.forSelect(WhitelistInfo.class, req, paging).select(columnNames).findPaging(null, paging);
    }

    @Override
    @Operation(summary = STAT_ACTION)
    public int count(QueryWhitelistReq req){
    ...
    @Operation(summary = QUERY_ACTION)
    @Override
    public WhitelistInfo findOne(QueryWhitelistReq req){
        return simpleDao.findOneByQueryObj(req);
    }

    @Operation(summary = QUERY_ACTION)
    @Override
    public WhitelistInfo findUnique(QueryWhitelistReq req){
