 ...
 * 客户端文件-服务实现
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年11月23日 下午11:58:32, 代码生成哈希校验码：[d84c8c6c3be0e3100c1effd1516a0aee]，请不要修改和删除此行内容。
 *
 */
...
public class AppClientFileServiceImpl extends BaseService implements AppClientFileService {
    ...
    @Override
    //@CacheEvict(condition = "#isNotEmpty(#req.id) && #result", key = E_AppClientFile.CACHE_KEY_PREFIX + "#req.id")
    @Transactional
    ...
    @Override
    //@Cacheable(condition = "#isNotEmpty(#id)", unless = "#result == null ", key = E_AppClientFile.CACHE_KEY_PREFIX + "#id")
    ...
    @Override
    //只更新缓存
    //@CachePut(unless = "#result == null" , condition = "#isNotEmpty(#req.id)" , key = E_AppClientFile.CACHE_KEY_PREFIX + "#req.id")
    ...
    @CacheEvict(condition = "#isNotEmpty(#key)", key = E_AppClientFile.CACHE_KEY_PREFIX + "#key")
