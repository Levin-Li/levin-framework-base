 ...
 * 客户端文件-服务实现
 *
 * @author Auto gen by simple-dao-codegen, @time: 2023年11月17日 上午2:26:21, 代码生成哈希校验码：[a678b6186ee1945277c126fa566a86f7]，请不要修改和删除此行内容。
 */
...
public class AppClientFileServiceImpl extends BaseService implements AppClientFileService {
    ...
    @Override
    // @CacheEvict(condition = "#isNotEmpty(#req.id)", key = E_AppClientFile.CACHE_KEY_PREFIX +
    // "#req.id")
    @Transactional(rollbackFor = RuntimeException.class)
    ...
    public PagingData<SimpleAppClientFileInfo> simpleQuery(
            QueryAppClientFileReq req, Paging paging) {
    ...
    // E_AppClientFile.CACHE_KEY_PREFIX + "#id")
    ...
    // E_AppClientFile.CACHE_KEY_PREFIX + "#req.id")
